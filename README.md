...
- [Dokumentace k prvnímu projektu do předmětu IPK](#dokumentace-k-prvnímu-projektu-do-předmětu-ipk)
  ...
  - [Návratové hodnoty programu](#návratové-hodnoty-programu)
...
Implementace je tvořena pomocí nízkoúrovňových síťových operací, využívající **RAW sokety** k manuální kosntrukci hlaviček u konkrétních protokolů a **epoll** pro efektivní čekání na odpovědi.
...
3. Pokud není získaná odpoveď, po vypršení timeout a to ani po **TCP Retransmission** (Opětovném zaslání) je port **filtrovaný (filtered)**.
...
Soket je programové komunikační rozhraní uzluz pro příjem a odeslání dat po sítí. Soket obsahuje informace potřené pro přenos dat a datové spojení.[6]
...
```c
IPK01
├── img/                             // Složka s obrázky k dokumentaci
...
│   ├── scanner.hpp                  // Deklarace abstraktní trídy Scanner její potomků
...
2. Způsob zadávání portů ze zadání, **nelze kombinovat.**
...
Testování nevalidních vstupů, bylo vykonáno pomocí bash skriput v `tests/parse/parse.sh`, pokud je **program sestaven** pomocí `make`, lze tento testovací skript spusit pomocí `Makefile`.

```bash
make test_input
```

Tento skript byl spuštěn jak na lokálním tak vlastněném fyzickém stroji.
